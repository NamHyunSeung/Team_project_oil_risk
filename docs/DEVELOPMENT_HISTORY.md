...
## Phase 19: 충격 후 레짐 캡 추가 (2026-06-04)

### 문제

4/8에 하루 -16.4% 급락 이후, 극단값 피처(mom_5d, vol_5d)에 모델이 과반응해
4/13 D+1 예측 $82.25 (실제 $91.28, 오차 +$9.03) 발생.
기존 `compute_error_spike_blend()`는 `|error|>$5 연속 3일` 조건으로 4/8 충격 직후
오차 소액 구간에서 미발동.
...
|------|-----------|------|------|------|
| 4/8  | -16.4%    | —    | $94.41 | — |
| 4/13 | —         | $82.25 | $91.28 | +$9.03 |

4/13 current price $99.08 기준 예측 변동률 -17% → 실제 -7.9%.
모델이 충격 피처 잔향에 과반응.

### 수정

최근 5영업일 내 단일 일간 변동 ≥8% 감지 시 D+1 변동폭 ±8% 캡 적용.
`full_df['WTI'].dropna().pct_change().abs().iloc[-5:].max()` 기준.
...
적용 위치: 감성 충격 보정(A5) 이후, sanity check 이전.

**threshold 8%에서 14%로 상향 조정 이유**:
최근 1년 데이터 전수 시뮬레이션 결과, threshold=8%는 3/6, 3/11, 3/12, 3/26 등 모델이
정확하게 큰 이동을 예측했던 날에도 캡이 발동되어 오차를 악화시킴 (총 오차 36.70→49.45).
threshold=14%는 4/8(16.4%) 수준의 극단 충격에만 발동되어 개선/악화 비율 1/0, 총 오차 12.04→5.45.

| threshold | 캡발동 | 개선/악화 | 총오차 변화 |
|-----------|--------|-----------|------------|
| 8% | 7일 | 2/5 | 36.70→49.45 |
| 12% | 3일 | 2/1 | 12.64→11.40 |
| 14% | 1일 | 1/0 | 12.04→5.45 |

### 검증

4/13 시뮬레이션 (threshold=14%):
- 4/8 변동 16.4% → 레짐 감지
- D+1 캡 하한: $96.57 × 0.92 = **$88.84**
- 예측 $82.25 → 클리핑 → **$88.84** (실제 $91.28, 오차 **-$2.44**)
- 기존 오차 +$9.03 → 개선 후 -$2.44

4/16 영향 없음:
- 4/16 기준 5영업일 내 최대 변동 7.87%(4/14) → threshold 14% 미달 → 미발동

### 백테스트 반영 (2026-06-04 추가)

`save_prediction_log()` 백테스트 루프(SARIMAX 테스트셋 순회)에 동일 충격 레짐 캡 로직 추가.
감성 충격 보정 이후, `p_err` 계산 이전 위치에 삽입.
- 창: `actual_prices[max(0, i-6):i]` (직전 최대 5영업일)
- threshold=14%, cap=±8% (live와 동일)
- 파이프라인 재실행 시 prediction_log.csv backtest 구간에 캡 적용 결과 자동 반영 → 대시보드 백테스트 차트에 표시

---

## Phase 20: 백테스트 대형 오차 원인 분석 및 개선 계획 (2026-06-04)

### 분석 배경

Phase 19 백테스트 반영 이후, 충격 레짐 캡이 발동되지 않은 $6+ 오차 케이스 5건 전수 분석.
...
5건 모두 충격 레짐 캡 미발동 (캡 창 최대 변동이 threshold 14% 미달).
...
```python
exog_cols = ['dxy_change', 'demand_shock', 'supply_shock', 'vix_change']
```

뉴스 감성, GPR, OVX가 SARIMAX exog에 없어 외부 충격이 예측값 자체에 반영되지 않는다.
감성은 post-processing ±3% 캡 보정으로만 사용 → -16% 충격 대응 구조적 불가.

4/7(월요일): 주말(4/5~4/6) 동안 복수 외생 이벤트 발생 후 월요일 개장에 누적 반영.
금요일(4/4) 예측 시점에 주말 이벤트 정보 없음 → 어떤 모델도 사전 인지 불가.

### 패턴별 해결 가능성

| 패턴 | 케이스 | 해결 가능성 |
|------|--------|-------------|
| 당일/주말 블랙스완 | 4/7, 4/16 | 불가 — 예측 시점에 정보 없음 |
| 급락 직후 반등 | 4/17 | 조건부 가능 — 전수 시뮬레이션 후 판단 필요 |
| 사전 시그널 없는 급변 | 4/28, 5/5 | 불가 — 뉴스 감성 중립(0.167/0.192), 피처 이상 없음 |

4/17 반등 보정 로직은 전날 급락 후 반등/비반등이 혼재(5/6: -0.3%, 5/20: -1.9%)해 단순 규칙 적용 시 다른 날 악화 위험. 전수 시뮬레이션 없이 추가 불가.

### 개선 계획 (미구현, 실험 예정)

| 우선순위 | 방법 | 예상 효과 | 리스크 |
|----------|------|-----------|--------|
| 1 | `vix_change` → `ovx_change` 교체 | SARIMAX가 원유 특화 변동성 직접 학습 | 낮음 — exog 슬롯 교체, 다중공선성 없음 |
| 2 | `news_sentiment_3d` exog 추가 | 감성이 예측값에 직접 반영, post-processing 보정 축소 | 중간 — 감성의 가격 선행성 실증 검증 필요 |

**전제조건**: 두 방법 모두 4/7(4/4 OVX), 4/16(4/15 OVX) 전날 값이 실제로 이상 시그널을 줬을 때만 효과적. 주말 이벤트 특성상 금요일 OVX가 낮았을 가능성 있어 효과 제한될 수 있음.

4/28, 5/5는 사전 시그널 없는 순수 외생 충격으로 어떤 피처 추가로도 해결 불가. CI 확대로만 대응 가능.

---

## Phase 21: 사용자 페이지 탭 분리 (2026-06-04)

### 변경 내용

`dashboard_v2.py`의 `render_user_page()` 단일 스크롤 구조를 탭 구조로 개편.
...
정보 밀도 과다 개선 — 스크롤 없이 현황(히어로+드라이버+지표) 즉시 파악, 상세 분석은 탭 전환.
...
## Phase 25: 구독 만료 자동 다운그레이드 (2026-06-05)

### 변경 내용

**dashboard_v2.py** — 구독 만료 처리 로직:

- 기존: 만료 시 로그아웃 + 접근 차단
- 변경: 만료 시 YAML `plan → free`, `subscription_expiry → 2099-12-31` 자동 업데이트 후 무료 플랜으로 계속 사용 가능
- 관리자 계정은 다운그레이드 대상 제외

### 목적

만료 후 관리자 개입 없이 서비스 접근성 유지, 갱신 유도 UX 개선.

---

## Phase 24: 회원가입 기능 구현 (2026-06-05)

### 변경 내용

**dashboard_v2.py** — 로그인 화면:

- 로그인 폼 → `st.tabs(["🔐 로그인", "📝 회원가입"])` 구조로 교체
- 회원가입 폼: 아이디(영문+숫자 4~20자), 이름, 이메일, 비밀번호(8자+), 플랜 선택
- 검증: 아이디 형식, 중복, 비밀번호 길이·일치, 이메일 형식
- bcrypt 해시 저장, `joined_at` 필드 추가
- 무료 플랜 → 만료일 2099-12-31, 유료 플랜 → today+7일 체험 자동 설정

**dashboard_v2.py** — Tab5 사용자 관리:

- 계정 목록 dataframe에 `플랜`, `가입일` 컬럼 추가
- 계정 추가 폼에 `플랜` selectbox + `joined_at` 자동 설정

### 목적

신규 사용자가 관리자 개입 없이 직접 가입·플랜 선택 가능하도록 자체 가입 기능 추가.

---

## Phase 23: 로그인 화면 3-플랜 카드 개편 (2026-06-04)

### 변경 내용

**dashboard_v2.py** — 로그인 화면 (구독자 미로그인 상태 표시):

- 기존 단일 PRO 카드($79/월 + 5개 기능) → 3열 플랜 카드로 교체
- **무료**: ₩0/월 — 리스크 등급, 핵심 지표, 가격 히스토리, 뉴스 키워드 5개
- **일반**: ₩490,000/월 — 무료 포함 + D+1~7 예측, 신뢰구간, 워드클라우드, 이메일 알람
- **프로**: ₩1,290,000/월 — 일반 포함 + 예측 성과 분석, 알람 임계값 설정, CSV 다운로드
- 로그인 폼 중앙 정렬 유지 (카드 아래)
- min-height:430px으로 3개 카드 높이 통일

### 목적

구독 플랜별 제공 기능을 로그인 전 단계에서 명확히 제시하여 전환율 향상.
...
## Phase 28: 발표 자료 준비 (2026-06-05)

### docs/generate_timeline_biz.py (신규)

- 기존 `generate_timeline.py`(기술 카테고리)와 별도로 발표용 비즈니스 버전 타임라인 생성 스크립트 추가
- 카테고리: `예측 정확도` / `리스크 감지` / `운영 안정화` / `비즈니스` (4색 구분)
- 핵심 마일스톤 3개 강조: Ph.13~14 "예측 오차 40% 개선", Ph.16 "모델 신뢰도 확보", Ph.26 "SaaS 완성"
- 주차별 배경 밴드(1~4주차) + 마일스톤 금색 글로우 효과
- 출력: `docs/timeline_biz.png` (130dpi, 5874×2221px)

### 15분 발표 PPT 구성 (비즈니스 + 있어빌리티)

**관통 메시지**: "날씨 예보 없이 여행 안 가듯, 유가 예보 없이 사업하지 마세요"  
**발표 전제**: 청중은 기술보다 비즈니스 가치에 관심. "날씨 예보" 비유를 처음부터 끝까지 관통.

| # | 슬라이드 제목 | 시간 | 핵심 내용 |
|---|-------------|------|----------|
| 1 | 훅 — 2026-04-07, 하루 만에 $15 폭락 | 30초 | 뉴스 헤드라인 1장 + 숫자만. 침묵으로 시작 |
| 2 | 피해 규모 — 숫자로 공감 | 1분 | "대한항공 연료비 연간 3.2조 — 유가 $1 = 약 300억 손익 차이" (공시 기반 추정, 각주 명시) |
| 3 | 기존 방식의 한계 | 1분 | 뉴스·직관·엑셀 → 느리다 / 주관적 / 전문지식 필요 |
| 4 | 제품 한 줄 소개 | 30초 | 대시보드 스크린샷 1장. 기술 설명 없이 화면만 |
| 5 | 작동 원리 — 3중 레이어 다이어그램 | 1분30초 | "기상청처럼 AI가 500개 신호를 읽어 7일 유가를 예보" + XGBoost 앙상블·SARIMAX·실시간 감성 분석 키워드 1줄 (설명 생략) |
| 6 | 핵심 기능 3가지 | 1분 | 7일 예측 / 리스크 등급 알림 / 블랙스완 조기 경보 |
| 6-A | 데이터 출처 — 공신력 | 45초 | EIA·CFTC·NewsAPI·Yahoo. "골드만삭스 원자재 리서치팀과 동일한 공식 데이터" |
| 6-B | 한계도 공개 — 투명성이 신뢰 | 45초 | 잘 맞는 상황 vs 솔직한 한계 병렬 제시. "100% 예측은 거짓말 — 우리는 언제 조심해야 하는지를 알려드립니다" |
| 7 | 성능 검증 — 벤치마크 비교 | 1분 | Persistence 1.000 / ETS·VAR ~1.000 / **우리 시스템 0.602** → "40% 개선, Walk-forward Validation 기준" |
| 8 | 타겟 고객 — 포지셔닝 맵 | 1분30초 | 복잡함·가격 2축 맵: Bloomberg Terminal vs 우리 vs 엑셀. "전문성은 Bloomberg급, 접근성은 엑셀급" |
| 9 | 수익 모델 — Freemium → SaaS | 1분 | ₩0 / ₩490,000 / ₩1,290,000. "Pro 100명 = 월 1.3억" |
| 9-A | 시장 규모 — TAM/SAM/SOM | 1분 | TAM $14.2B / SAM ~4,200개사 / SOM 200개사 × ₩49만 = 월 1억 |
| 10 | 왜 지금, 왜 우리 — 타임라인 | 1분 | `timeline_biz.png` 삽입. "Lean 스프린트 4회 — MVP to Production" |
| 11 | 시연 예고 | 15초 | "지금 실제로 작동하는 화면을 보여드립니다" |
| 12 | 마무리 · CTA | 30초 | QR 코드 + "날씨 예보 없이 여행 안 가듯…" |

**시연 5분**: 로그인(10초) → 오늘 예측+리스크 등급(1분) → 7일 차트(1분) → HIGH 리스크 시나리오(1분) → 구독 플랜 화면(30초) → Q&A

#### 슬라이드 6-A 상세 — 데이터 신뢰성

| 데이터 소스 | 제공 기관 | 활용 |
|------------|----------|------|
| 원유 재고·수급 통계 | EIA (미국 에너지청) | 수급 신호 |
| 시장 포지션 (투기/헤지) | CFTC (미 상품선물거래위원회) | 시장 심리 |
| 실시간 지정학 뉴스 | NewsAPI · RSS 500+ 소스 | 감성 분석 |
| 시장 가격·변동성 | Yahoo Finance | 가격 피처 |

#### 슬라이드 6-B 상세 — 투명한 한계 공개

| 잘 맞는 상황 | 솔직한 한계 |
|-------------|------------|
| 평시 가격 흐름 예측 | 주말 블랙스완 (관세·전쟁 이벤트) |
| 방향성 61% 적중 | 당일 급락 — 전일까지 신호 없음 |
| 오차 평균 $2 이내 | 지정학 충격 직후 72시간 |

> 핵심: 한계를 먼저 인정하면 나머지 수치의 신뢰도가 올라감. 이 슬라이드를 가장 자신 있게 발표.

#### 슬라이드 7 상세 — 성능 벤치마크 비교

| 모델 | MASE | 비고 |
|------|------|------|
| Persistence (아무것도 안 한 것) | 1.000 | 기준선 |
| ETS / VAR (전통 통계 모델) | ~1.000 | 기준선 수준 |
| **우리 시스템** | **0.602** | **40% 개선** |
...
| 구분 | 규모 | 설명 |
|------|------|------|
| TAM | $14.2B | 전 세계 에너지 리스크 관리 솔루션 시장 (2025) |
| SAM | ~4,200개사 | 국내 유류 다소비 기업 (물류·항공·정유) |
| SOM | 월 ~1억 | 초기 타겟 200개사 × ₩490,000 |

#### 있어빌리티 언어 치환표

| 일반 표현 | 발표용 표현 |
|----------|-----------|
| "잘 맞습니다" | "검증된 예측 정확도 (Walk-forward Validation)" |
| "자동으로 돌아갑니다" | "무중단 자동화 파이프라인 (Zero-downtime)" |
| "구독 플랜 3개" | "Freemium → SaaS 전환 수익 모델" |
| "4주 만에 만들었습니다" | "Lean 스프린트 4회 — MVP to Production" |
| "한계가 있습니다" | "예측 신뢰 구간 및 적용 범위 명시" |

---

## Phase 30: SARIMAX 백테스트 날짜-값 정렬 버그 수정 (2026-06-08)

### 문제

5/20 백테스트 행의 오차가 -11.64로 비정상적으로 컸음 (직전/직후 행은 -1~-5 수준).

### 원인

`_to_bday()`로 영업일 캘린더로 확장된 `full_wti`(공휴일도 ffill로 채워짐)에서 예측값을
포지션 슬라이싱(`full_wti.values[-n_test_sx:]`)으로 추출한 반면, 비교 대상인 실측값/날짜
라벨은 원본 `sx_test.index`(실제 거래일만 포함)에서 날짜 기준으로 추출 — 두 시리즈의
길이가 달라 `pred_price[i]`가 `_dates_eval[i]`와 다른 날짜의 값을 담는 정렬 오류 발생
(예외 없이 조용히 발생). 5/20 행에 찍힌 예측값 107.99는 실제로 5/15 시점 예측값이었음.

### 수정

`oil_risk_mvp.py:2911-2934` — 포지션 슬라이싱을 날짜 인덱스 기반 정렬로 교체:
`pred_obj.predicted_mean`(DatetimeIndex 보유)과 `full_wti - full_wti.shift(1)`
(pandas Series 덧셈으로 날짜 자동 정렬)을 조합한 뒤 `.reindex(sx_test.index)`로
명시적 날짜 매핑. 길이 불일치에 의존하지 않는 방식으로 동일 메커니즘의 재발을 차단.

### 검증 (파이프라인 전체 재실행, backtest 89건)

| 구간 | 수정 전 오차 | 수정 후 오차 |
|------|------------|------------|
| 5/20 | **-11.64** (비정상) | **-1.94** (정상 범위) |
| 전체 평균 \|오차\| | 3.567 | 2.994 |
| 전체 최대 \|오차\| | 14.59 | 15.17 |

수정 후 새로운 최대오차(4/7 -15.17, 4/1 +11.84, 3/9 -11.61)는 정렬 오류가 아니라
변동성이 컸던 시기의 정상적인 1-step 모델 예측 미스 (Phase 20 분석과 일치).

`_to_bday` 패턴은 SARIMAX 평가 구간에만 사용되어, 동일 유형의 latent 정렬 버그가
다른 곳에 있을 가능성은 낮음 (grep으로 확인).
