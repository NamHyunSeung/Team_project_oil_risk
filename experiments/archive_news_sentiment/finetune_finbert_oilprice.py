"""
FinBERT LoRA Fine-tuning for Oil Price Direction — 7th attempt (OilPrice 44k headlines)
- Base: ProsusAI/finbert
- LoRA: rank=16, alpha=32, target=[query, value]
- Binary classification: up(1) / down(0), label = D+1 Brent direction (±0.5% threshold)
- Data: output/oilprice_finetune_data.csv (27,668 rows, 2010-2026)
- Split: time-series 80/10/10 (train/valid/test)
- Output: output/finbert_oilprice/
"""
import sys, warnings
...
# ── Config ────────────────────────────────────────────────────────────────────
LABELED_FILE = Path("output/oilprice_finetune_data.csv")
MODEL_DIR    = Path("output/finbert_oilprice")
